...
public class MainActivity extends Activity {
    ...
    private void startNativeLogin(final String username, final String password) {
        final String u=username==null?"":username.trim(); final String p=password==null?"":password;
        ...
        executor.execute(() -> {
            ...
            } catch(Exception e){final String msg=e.getMessage()==null?"Login failed. Check internet access and Google authorization.":e.getMessage();runOnUiThread(()->{Toast.makeText(MainActivity.this,msg,Toast.LENGTH_LONG).show();loadHome();});}
        });
    }

    private void startNativeReportLoad(final String date) {
        Toast.makeText(this,"Loading report "+date+"…",Toast.LENGTH_SHORT).show();
        executor.execute(() -> {try{String report=fetchReportByDate(date);JSONArray reports=listReports();runOnUiThread(()->loadDashboardWithData(reports,date,report));}catch(Exception e){final String msg=e.getMessage()==null?"Unable to load the selected report.":e.getMessage();runOnUiThread(()->Toast.makeText(MainActivity.this,msg,Toast.LENGTH_LONG).show());}});
    }
    ...
    public class AndroidBridge{
        ...
        @JavascriptInterface public void login(String username,String password){startNativeLogin(username,password);}
        @JavascriptInterface public void getLatestReport(){executor.execute(()->{try{JSONArray reports=listReports();if(reports.length()==0)throw new Exception("No daily reports found on Google Drive.");String date=reports.getJSONObject(0).getString("date");String report=fetchReportByDate(date);runOnUiThread(()->loadDashboardWithData(reports,date,report));}catch(Exception e){runOnUiThread(()->Toast.makeText(MainActivity.this,e.getMessage(),Toast.LENGTH_LONG).show());}});}
    ...
    private JSONArray listReports()throws Exception{
        String token=getAccessToken();String q="'"+DRIVE_FOLDER_ID+"' in parents and trashed = false";String url="https://www.googleapis.com/drive/v3/files?q="+URLEncoder.encode(q,"UTF-8")+"&orderBy=modifiedTime desc&pageSize=100&fields=files(id,name,modifiedTime)";JSONObject listed=httpJson("GET",url,token,null);JSONArray source=listed.optJSONArray("files");JSONArray reports=new JSONArray();if(source==null)return reports;ArrayList<JSONObject> temp=new ArrayList<>();
        for(int i=0;i<source.length();i++){JSONObject f=source.getJSONObject(i);String name=f.optString("name","");if(!name.matches("session_\\d{4}-\\d{2}-\\d{2}\\.json"))continue;JSONObject item=new JSONObject();item.put("date",name.substring(8,18));item.put("name",name);item.put("id",f.optString("id",""));item.put("modifiedTime",f.optString("modifiedTime",""));temp.add(item);}temp.sort((a,b)->b.optString("modifiedTime","").compareTo(a.optString("modifiedTime","")));for(JSONObject item:temp)reports.put(item);return reports;
    }
